...
export default function AnimatedServiceCard({
  ...
  return (
    <Tag
      onClick={onClick}
      className={cn(
        'group relative h-full overflow-visible rounded-2xl border border-surface/30 bg-surface/30 transition-colors duration-300 hover:border-accent/40 hover:bg-surface/60 hover:shadow-[0_8px_40px_hsl(var(--accent)/0.12)] active:border-accent/40 active:bg-surface/60',
        className,
      )}
    >
      <GlowingEffect
        ...
        borderWidth={4}
      />
      <div className="absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100 pointer-events-none max-md:hidden rounded-[inherit]">
      ...
      <div className="absolute inset-0 opacity-0 transition-all duration-700 group-hover:opacity-100 pointer-events-none max-md:hidden rounded-[inherit]">
